...
void Ge_encoder_odometry::estimate(SensorData *rawdata) {
  ...
  if (m_current_t_read - m_prev_t_read > 500000) {
    //RCLCPP_WARN("gap is too big, current_time("
...
//RCLCPP_WARN(logger_, "the Record file is not open!");
    }
  }
}

void Ge_encoder_odometry::init(bool m_if_record, const std::string & m_record_file_path) {
  m_isInit = true;
  ...
  if (m_if_record) {
	...
	//RCLCPP_INFO(logger_, "file_path: %s", file_path.c_str());
...
bool Ge_encoder_odometry::add_ticks(SensorData::Ticks &ticksdata) {
  ...
  if (m_imu_record_.imu_buffer_.empty() ||
    ...
    //RCLCPP_WARN(logger_, "%ld: the ticks is elear than imu", ticksdata.TimeStamp);
