...
// For logging in ROS2, include rclcpp if you want
...
class Ge_encoder_odometry {
 public:
  ...
  // Use this if you want to support ROS2-style logging
  void set_logger(const rclcpp::Logger & logger) { logger_ = logger; }
 ...
 private:
  ...
  // Pass parameters into init, don't use ros::NodeHandle
  void init(bool if_record = false, const std::string & record_file_path = "/home/linaro/ros2_ws/");
  ...
  // For ROS2 logging support
  rclcpp::Logger logger_ = rclcpp::get_logger("Ge_encoder_odometry");
};
